...
- A real cross-user leak was traced to Hermes's single global `memories/USER.md`, which mixed Vandy and Khusha facts and caused Ted to mention Bangalore to Ankita. Built-in global memory and user-profile injection are now disabled for the public beta, and `SOUL.md` forbids facts from other chats and requires hypothetical recaps to start with `Example only:`. Existing live conversation sessions remain separate; durable per-user memory still requires the planned Convex path.
...
- Beta track: verify the new opener in a fresh real WhatsApp thread, then send the tester invitation in `docs/BETA_TESTER_FLOW.md` to known adult users.
